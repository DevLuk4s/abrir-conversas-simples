// pipeline.js - porta do prospeccao-pipeline/processar.js pro navegador.
// Expõe window.Pipeline.processar(brutos) -> lista de leads com mensagem pronta.
// `brutos` é a matriz CSV completa (linha 0 = cabeçalho).
...
(function () {
  ...
  const BLOCOS = {
    ...
    empresarial: {
      ...
      f: [
        " Um site ajudaria a captar orçamento direto, sem depender de quem passa por ali. Posso te mandar uma proposta sem compromisso?",
  ...
  function processar(brutos) {
    if (!brutos || brutos.length < 2) return [];

    contadorGlobal = { valor: 0 };
    for (const k in combosUsados) delete combosUsados[k];
    ...
    if (["title", "name", "nome"].every((n) => col[n] === undefined)) return [];
    if (["phone", "phoneunformatted", "telefone", "telefone_formatado"].every((n) => col[n] === undefined)) return [];
    ...
    for (let i = 1; i < brutos.length; i++) {
      const r = brutos[i];
      if (!r || r.length < cabecalho.length) continue;
      ...
      if (numeros.length > 0 && numeros.length < 12) numeros = "";

      const endereco = valor(r, ["address", "street", "cidade", "bairro"]);
      const notaTextoBruto = valor(r, ["totalscore", "nota"]).trim();
      const avalTexto = valor(r, ["reviewscount", "avaliacoes"]).replace(/\D/g, "");
      const nota = notaTextoBruto === "" ? NaN : parseFloat(notaTextoBruto.replace(",", "."));
